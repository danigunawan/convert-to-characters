...
$(document).ready(function () {
  ...
  function translateInputText() {
    var trimmedInput = trimAndLowerCaseText($("#input").val());
    disableButtons();
    if (trimmedInput) {
  ...
  function setRecognizedLanguage(language) {
    var trimmedLanguage = trimAndLowerCaseText(language);
  ...
  function setTranslator(translator) {
    var trimmedTranslator = trimAndLowerCaseText(translator);
    if ($.inArray(trimmedTranslator, lowerCaseTranslators) !== -1) {
      var new_value = trimmedTranslator === "google" ? "google_translate" : "yandex_translate";
    ...
    $.ajax({
      ...
      success: function success(data) {
        ...
        if ($.trim(data.translated_text).length) {
          ...
          var voiceLanguage = "Spanish Female";
